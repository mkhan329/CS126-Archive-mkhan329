...
public class MineopolyMain {
    private static final int DEFAULT_BOARD_SIZE = 20; //default 20
    ...
    public static void main(String[] args) {
        if (TEST_STRATEGY_WIN_PERCENT) {
            MinePlayerStrategy yourStrategy = new MoizKhanStrategy(); // Replace this with your strategy
    ...
    private static void playGameOrReplay() {
        ...
        if (savedReplayFilePath == null) {
            ...
            MinePlayerStrategy blueStrategy = new MoizKhanStrategy(); // Replace this with your strategy
            MinePlayerStrategy redStrategy = new MoizKhanStrategy();
    ...
    private static double getStrategyWinPercent(MinePlayerStrategy yourStrategy, int boardSize) {
        ...
        for (int i = 0; i < numTotalRounds; i++) {
            MoizKhanStrategy blueStrategy = new MoizKhanStrategy();
            MinePlayerStrategy redStrategy = new RandomStrategy();
            long randomSeed = System.currentTimeMillis();
            gameEngine = new GameEngine(DEFAULT_BOARD_SIZE, redStrategy, blueStrategy, randomSeed);
            gameEngine.runGame();
            numRoundsWonByMinScore += blueStrategy.gameWon() ? 1 : 0;
        }

        assert ((double) numRoundsWonByMinScore) / numTotalRounds >= 0.99 : "Strategy must win at least 99% of the games!";
